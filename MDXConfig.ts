...
import { githubDarkDimmed } from '@/styles/github-dark-dimmed';
import { MDXRemoteProps, MDXRemote } from 'next-mdx-remote/rsc';

const MDXInit = async (mdxSource: string) => {
  ...
  return (tree: any) => {
    visit(tree, 'element', (node: any, index: any, parent: any) => {
      if (['h1', 'h2', 'h3', 'h4', 'h5', 'h6'].includes(node.tagName)) {
        ...
        parent.children[index] = headingTag;
      }
    });
  };
};
...
function rehypePrettyCodeBlocks() {
  ...
  return (tree: any) => {
    visit(tree, 'element', (node: any, index: any, parent: any) => {
      ...
      if (Object.keys(code.properties).length === 0) {
        code.properties['className'] = ['language-text'];
        code.data = { meta: `chrome=no` };
      }
      ...
      const lang = className.find(
        (value: any) => value.slice(0, 9) === 'language-',
      );
      const header = [];
      const footer = [];
      ...
      if (metaProps.path) {
        ...
        header.push(
          h(
            ...
            languageNames[
              language.split(':')[0] as keyof typeof languageNames
            ] || language,
          ),
        );
      }
